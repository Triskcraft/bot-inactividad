...
export class RoleService {
  ...
  async persistSnapshot(guild_id: string, role_id: string, inactive_count: number, active_count: number) {
  ...
  async getSnapshots(guild_id: string, sinceDays = 30) {
    const since = new Date(Temporal.Now.instant().subtract({ hours: sinceDays * 24 }).epochMilliseconds)
